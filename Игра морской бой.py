...
class Dot: # класс точка
    ...
    def __repr__(self): # метод вывода точек в консоль
...
class BoardUsedException(BoardException): # клетка занята
    def __str__(self):
        return "Вы уже стреляли в эту клетку"

class BoardWrongShipException(BoardException): # размещение кораблей
    pass
...
class Ship: # класс корабля
    ...
    def shooten(self, shot): # попадание
...
class User(Player):
    def ask(self):
        while True:
            cords = input("Введите 2 координаты через пробел. Ваш ход: ").split()
